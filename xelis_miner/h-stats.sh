...
ac_cpu=`sed "s/$ESC\[[^Gm]*[Gm]//g;s/$ESC%G//g;s/$CR//g" ${CUSTOM_LOG_BASENAME}.log | tail -n100 | grep -w "Accepted" | awk '{print $9}' | tail -n1`
rj_cpu=`sed "s/$ESC\[[^Gm]*[Gm]//g;s/$ESC%G//g;s/$CR//g" ${CUSTOM_LOG_BASENAME}.log | tail -n100 | grep -w "Rejected" | awk '{print $12}' | tail -n1`
fnd_ac_gpu=`sed "s/$ESC\[[^Gm]*[Gm]//g;s/$ESC%G//g;s/$CR//g" ${CUSTOM_LOG_BASENAME}.log | grep 'CUDA' | grep 'Found' | awk '{print $6}' | tail -n1`
if [[ $fnd_ac_gpu != "" ]];then
ctr_fnd_ac_gpu=`cat ${CUSTOM_CONFIG_FILENAME}_gpufnd | grep ${fnd_ac_gpu} | wc -l `
  if [[ $ctr_fnd_ac_gpu -eq 0 ]];then
echo "Found ${fnd_ac_gpu}" >> ${CUSTOM_CONFIG_FILENAME}_gpufnd
  fi
fi 

ac_gpu=`cat ${CUSTOM_CONFIG_FILENAME}_gpufnd | grep 'Found' | wc -l`
