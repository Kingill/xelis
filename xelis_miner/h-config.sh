...
eval "rm -rf ${CUSTOM_CONFIG_FILENAME}_CONF"
eval "rm -rf ${CUSTOM_CONFIG_FILENAME}_CPU"
eval "rm -rf ${CUSTOM_CONFIG_FILENAME}_GPU"
if [[ ! -z $CUSTOM_USER_CONFIG ]]; then

echo "${CUSTOM_USER_CONFIG}" > "${CUSTOM_CONFIG_FILENAME}_CONF"
nv=`cat ${CUSTOM_CONFIG_FILENAME}_CONF | tail -n10 | grep 'nvtool' | tail -n1`
if [[ $nv != "" ]];then
eval $nv 
fi
oc_cpu=`grep -oP "n \K\d+" "${CUSTOM_CONFIG_FILENAME}_CONF" | tail -n1`
if [[ $oc_cpu != "" ]];then
Settings_cpu="-n $oc_cpu"
fi
oc_cpu2=`cat ${CUSTOM_CONFIG_FILENAME}_CONF | tail -n10 | grep 'threads' | tail -n1`
if [[ $oc_cpu2 != "" ]];then
Settings_cpu=$oc_cpu2
fi
oc_gpu=`cat ${CUSTOM_CONFIG_FILENAME}_CONF | tail -n10 | grep 'boost' | tail -n1`
if [[ $oc_gpu != "" ]];then
Settings_gpu=$oc_gpu
fi
fi
echo "" > ${CUSTOM_CONFIG_FILENAME}_gpufnd
